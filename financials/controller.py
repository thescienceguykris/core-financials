...
def getAuthCodeFromDB(username):
    ...
    token = (
        models.tokens.query.filter_by(belongs_to=user)
        .order_by(models.tokens.expires_at.desc())
        .first()
    )
...
def accounts(db, username, auth_code):
    ...
    for account in accounts:
        db.session.add(models.bank_account(account.get("id"), current_user.id))
...
def transactions(db, username, account_id, auth_code, younger_than_300):
    current_user = models.user.query.filter_by(username=username).first()
    account = models.bank_account.query.filter_by(
        id=account_id, belongs_to_id=current_user.id
    ).first()

    if account == None:
        return "Invalid account", 400

    params = {"account_id": account.monzo_account_id}
    ...
    response = util.get_monzo_request(
        "/transactions", auth_code=auth_code, params=params
    ).json()
    ...
    for transaction in transactions:
        ...
        try:
            created_date = datetime.datetime.strptime(
                transaction.get("created"), "%Y-%m-%dT%H:%M:%S.%fZ"
            )
        except ValueError:
            created_date = datetime.datetime.strptime(
                transaction.get("created"), "%Y-%m-%dT%H:%M:%SZ"
            )

        try:
            updated_date = datetime.datetime.strptime(
                transaction.get("updated"), "%Y-%m-%dT%H:%M:%S.%fZ"
            )
        except:
            updated_date = (
                datetime.datetime.strptime(
                    transaction.get("updated"), "%Y-%m-%dT%H:%M:%SZ"
                ),
            )

        settled_date = None
        if transaction.get("settled") == None:
            settled_date = datetime.datetime.strptime(
                transaction.get("settled"), "%Y-%m-%dT%H:%M:%S.%fZ"
            )
        ...
        existing_transaction = (
            update_session.query(models.transaction)
            .filter_by(monzo_transaction_id=transaction.get("id"))
            .first()
        )
        ...
        if (existing_transaction != None) and (
            (existing_transaction.updated != updated_date)
            or (existing_transaction.spend_category != transaction.get("category"))
        ):
